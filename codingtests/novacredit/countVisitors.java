...
class CountVisitors {
    ...
    private static void logHit() {
        Long hitsThisSecond = CountVisitors.hitsEachSecond.get(CountVisitors.hitsEachSecond.size() - 1);
        CountVisitors.hitsEachSecond.set(CountVisitors.hitsEachSecond.size() - 1, hitsThisSecond + 1);
        logHitToHistory();
    }
    ...
    private static Long getHits() {
        return CountVisitors.hitsEachSecond.stream().mapToLong(a -> a).sum();
    }
    ...
    private static void initializeMinuteCounter() throws InterruptedException {
        CountVisitors.hitsEachSecond.add(0L); // Add a zeroed entry at the end of the list (first entry of the list)
        ...
        hitCounterLimiter.scheduleAtFixedRate(new TimerTask() {
            ...
            @Override
            public void run() {
                int firstEntry = CountVisitors.hitsEachSecond.size() > 59 ? CountVisitors.hitsEachSecond.size() - 59
                        : 0;
                CountVisitors.hitsEachSecond = new ArrayList<Long>(
                        CountVisitors.hitsEachSecond.subList(firstEntry, CountVisitors.hitsEachSecond.size()));
                CountVisitors.hitsEachSecond.add(0L); // Add a new zeroed entry to the end of the list
    ...
    /**
     * This is here just for demonstrating the functionality, not really part of the
     * solution.
    ...
    private static void runDemo() throws InterruptedException {
        ...
        hitChecker.scheduleAtFixedRate(new TimerTask() {
            ...
            @Override
            public void run() {
                System.out.println("Seconds logged: " + CountVisitors.hitsEachSecond.size()); // Debug info for #
                                                                                              // seconds
                // recorded
                System.out.println("\ngetHits returns: " + CountVisitors.getHits());
    ...
    private static void logHitToHistory() {
        ...
        CountVisitors.hitHistoryArray[now.get(Calendar.YEAR) - 2019][now.get(Calendar.MONTH)][now
                .get(Calendar.DATE)][now
                        .get(Calendar.HOUR_OF_DAY)][now.get(Calendar.MINUTE)] += 1;
    }

    public static int getHitsThisHour() {
        Calendar now = Calendar.getInstance();
        return IntStream.of(CountVisitors.hitHistoryArray[now.get(Calendar.YEAR) - 2019][now.get(Calendar.MONTH)][now
                .get(Calendar.DATE)][now.get(Calendar.HOUR_OF_DAY)]).sum();
    }
    ...
    public static int getHitsThisDate() {
        ...
        for (int hour = 0; hour < 24; hour++) {
            sum += IntStream
                    .of(CountVisitors.hitHistoryArray[now.get(Calendar.YEAR) - 2019][now.get(Calendar.MONTH)][now
                            .get(Calendar.DATE)][hour])
                    .sum();
        }
        return sum;
    }
    ...
    public static int getHitsThisMonth() {
        ...
        for (int date = 0; date < 31; date++) {
            for (int hour = 0; hour < 24; hour++) {
                sum += IntStream.of(
                        CountVisitors.hitHistoryArray[now.get(Calendar.YEAR) - 2019][now
                                .get(Calendar.MONTH)][date][hour])
                        .sum();
            }
        }
        return sum;
    ...
    public static int getHitsThisYear() {
        ...
        for (int month = 0; month < 12; month++) {
            for (int date = 0; date < 31; date++) {
                for (int hour = 0; hour < 24; hour++) {
                    sum += IntStream.of(CountVisitors.hitHistoryArray[now.get(Calendar.YEAR) - 2019][month][date][hour])
                            .sum();
                }
            }
        }
    ...
    public static int getHitsFor(int year, int month, int date, int hour) {
        ...
        // if all params != null
        return IntStream.of(CountVisitors.hitHistoryArray[year - 2019][month][date][hour]).sum();
    }
}
